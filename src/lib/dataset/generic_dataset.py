...
class GenericDataset(data.Dataset):
    ...
    _eig_vec = np.array(
        [
            [-0.58752847, -0.69563484, 0.41340352], [-0.5832747, 0.00994535, -0.81221408],
    ...
    def _flip_anns(self, anns, width):
        for k in range(len(anns)):
            ...
            if 'hps' in self.opt.heads and 'keypoints' in anns[k]:
                keypoints = np.array(anns[k]['keypoints'],
                                     dtype=np.float32).reshape(self.num_joints, 3)
    ...
    def _get_input(self, img, trans_input):
        inp = cv2.warpAffine(
            img, trans_input, (self.opt.input_w, self.opt.input_h), flags=cv2.INTER_LINEAR
        )
    ...
    def _format_gt_det(self, gt_det):
        ...
        gt_det = {
            k: np.array(gt_det[k], dtype=np.float32)
            for k in gt_det
        }
        return gt_det
