...
class Detector(object):
    ...
    def _get_default_calib(self, width, height):
        calib = np.array(
            [
                ...
                [0, self.rest_focal_length, height / 2, 0], [0, 0, 1, 0]
            ]
        )
        return calib
    ...
    def _sigmoid_output(self, output):
        ...
        # if 'class' in output:
        #     output['class'] = output['class'].sigmoid_()
        return output
    ...
    def _flip_output(self, output):
        ...
        single_flips = [
            'ltrb', 'nuscenes_att', 'velocity', 'ltrb_amodal', 'reg', 'hp_offset', 'rot',
            'tracking', 'pre_hm'
        ]
        for head in output:
    ...
    def post_process(self, dets, meta, scale=1):
        dets = generic_post_process(
            self.opt, dets, [meta['c']], [meta['s']], meta['out_height'], meta['out_width'],
            self.opt.num_classes, [meta['calib']], meta['height'], meta['width']
        )
        ...
        if len(results) > 0 and \
          'dep' in results[0] and 'alpha' in results[0] and 'dim' in results[0]:
            debugger.add_3d_detection(
                image if not self.opt.qualitative else
                cv2.resize(debugger.imgs['pred_hm'], (image.shape[1], image.shape[0])),
                False,
                results,
                self.this_calib,
            ...
            debugger.add_bird_view(
                results, vis_thresh=self.opt.vis_thresh, img_id='bird_pred', cnt=self.cnt
            )
