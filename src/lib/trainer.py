...
class GenericLoss(torch.nn.Module):
    ...
    def forward(self, outputs, batch):
        ...
        for s in range(opt.num_stacks):
            ...
            regression_heads = [
                'reg', 'wh', 'tracking', 'ltrb', 'ltrb_amodal', 'hps', 'dep', 'dim',
                'amodel_offset', 'velocity'
            ]
            ...
            if 'hm_hp' in output:
                losses['hm_hp'] += self.crit(
                    output['hm_hp'], batch['hm_hp'], batch['hp_ind'], batch['hm_hp_mask'],
                    batch['joint']
                ) / opt.num_stacks
                if 'hp_offset' in output:
                    losses['hp_offset'] += self.crit_reg(
                        output['hp_offset'], batch['hp_offset_mask'], batch['hp_ind'],
                        batch['hp_offset']
                    ) / opt.num_stacks
    ...
    def debug(self, batch, output, iter_id, dataset):
        ...
        for i in range(1):
            ...
            if 'pre_img' in batch:
                ...
                pre_img = np.clip(((pre_img * dataset.std + dataset.mean) * 255), 0,
                                  255).astype(np.uint8)
            ...
            if 'rot' in opt.heads and 'dim' in opt.heads and 'dep' in opt.heads:
                ...
                det_gt = generic_post_process(
                    opt, dets_gt, batch['meta']['c'].cpu().numpy(),
                    batch['meta']['s'].cpu().numpy(), output['hm'].shape[2],
                    output['hm'].shape[3], self.opt.num_classes, calib
                )
