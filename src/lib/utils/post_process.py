...
def generic_post_process(
    ...
    for i in range(len(dets['scores'])):
        ...
        for j in range(len(dets['scores'][i])):
            ...
            # 这里将class的embedding换为了index
            item['class'] = int(np.argmax(dets['class'][i][j]))
            ...
            if 'hps' in dets:
                pts = transform_preds_with_trans(dets['hps'][i][j].reshape(-1, 2),
                                                 trans).reshape(-1)
                item['hps'] = pts
            ...
            if 'rot' in dets and 'dep' in dets and 'dim' in dets \
              and len(dets['dep'][i]) > j:
                if 'amodel_offset' in dets and len(dets['amodel_offset'][i]) > j:
                    ...
                    ct = transform_preds_with_trans(amodel_ct_output.reshape(1, 2),
